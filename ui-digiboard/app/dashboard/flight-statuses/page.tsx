...
import type { FlightStatus, FlightStatusPagination, FlightStatusFormData } from '@/types/flight-status';
...
export default function FlightStatusPage() {
  ...
  const [pagination, setPagination] = useState<FlightStatusPagination | null>(null);
  ...
  // Form States
  const [selectedStatus, setSelectedStatus] = useState<FlightStatus | null>(null);
  const [formData, setFormData] = useState<FlightStatusFormData>({
    name: '',
    color: '#000000'
  });
  ...
  const fetchStatuses = async (page = 1) => {
    ...
    try {
      ...
      const response = await fetchWithAuth(`http://localhost:8001/api/flight-statuses?${queryParams}`);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      const url = selectedStatus
        ? `http://localhost:8001/api/flight-statuses/${selectedStatus.id}`
        : 'http://localhost:8001/api/flight-statuses';
      ...
      closeModals();
      fetchStatuses(pagination?.current_page || 1);
    } catch (error) {
      console.error(error);
      alert('Failed to save flight status');
    } finally {
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetchWithAuth(`http://localhost:8001/api/flight-statuses/${selectedStatus.id}`, {
        method: 'DELETE',
      });
      ...
      console.error(error);
      alert('Failed to delete flight status');
    } finally {
      setFormLoading(false);
    }
  };
  ...
  const openEditModal = (status: FlightStatus) => {
  ...
  const openDeleteModal = (status: FlightStatus) => {
    setSelectedStatus(status);
    ...
    <ThemeProvider theme={lightTheme}>
      ...
      <Box sx={{ p: 4, bgcolor: 'background.default', minHeight: '100vh' }}>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', mb: 5 }}>
          <Box>
            ...
            <Typography variant="body1" color="text.secondary">
              Manage flight status options (e.g. On Time, Delayed, Cancelled)
            </Typography>
          </Box>
          <Button
            variant="contained"
        ...
        <Paper sx={{ p: 2, mb: 4 }}>
          <TextField
            fullWidth
            placeholder="Search flight statuses..."
            value={search}
          ...
          <Table>
            <TableHead>
              <TableRow>
                <TableCell
                  ...
                  sx={{ cursor: 'pointer' }}
                >
                  Status Name {sortBy === 'name' && (sortDirection === 'asc' ? '↑' : '↓')}
                </TableCell>
                <TableCell
            ...
            <TableBody>
              {loading ? (
                ...
                <TableRow>
                  <TableCell colSpan={3} align="center" sx={{ py: 8 }}>
                    <Typography color="text.secondary">No flight statuses found.</Typography>
                  </TableCell>
                </TableRow>
              ) : (
                statuses.map((status) => (
          ...
          {pagination && (
            <Box sx={{ p: 3, display: 'flex', justifyContent: 'center', borderTop: '1px solid', borderColor: 'divider' }}>
              <Pagination
                ...
                onChange={(_, page) => fetchStatuses(page)}
                color="primary"
                shape="rounded"
                size="large"
              />
          ...
          <form onSubmit={handleSubmit}>
            <DialogTitle sx={{ pt: 3, px: 3 }}>
              <Typography variant="h5" fontWeight="bold">
                {isAddModalOpen ? 'Add New Flight Status' : 'Edit Flight Status'}
              </Typography>
            </DialogTitle>
            <DialogContent sx={{ px: 3 }}>
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3, pt: 2 }}>
                ...
                <TextField
                  ...
                  sx={{ height: 56, '& input': { cursor: 'pointer' } }}
                />
              </Box>
            </DialogContent>
          ...
          <DialogContent>
            <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
              ...
              <Typography align="center" color="text.secondary">
                Are you sure you want to delete status <strong>{selectedStatus?.name}</strong>?
